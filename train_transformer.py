...
def train_transformer(data_path, save_path, epochs=5, batch_size=8, lr=1e-4):
    """Train the single layer transformer model on CoT data"""
    ...
    # Create model
    config = SingleLayerTransformerConfig(
    ...
    print(f"Training transformer on {len(train_data)} samples for {epochs} epochs")
...
def process_for_training(sample, tokenizer):
    ...
    if len(full_tokens) > 256:
        # Truncate question if too long, keep full answer
        max_question_len = 256 - len(answer_tokens)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_path", default="transformer_checkpoint", help="Path to save model")
